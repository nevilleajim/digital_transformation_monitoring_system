...
PROCESSED_FILE = PROJECT_ROOT / "data" / "processed" / "itu_datahub_clean.csv"

START_YEAR = 2010
END_YEAR = 2025

EXPECTED_INDICATORS = {
    "ITU.IT_NET_USER": {"name": "Individuals using the Internet", "min": 0, "max": 100},
    "ITU.IT_CEL_SETS_P2": {"name": "Mobile cellular subscriptions", "min": 0, "max": None},
    "ITU.IT_NET_BBND_P2": {"name": "Fixed broadband subscriptions", "min": 0, "max": None},
    "ITU.IT_MLT_MAIN_P2": {"name": "Fixed telephone subscriptions", "min": 0, "max": None},
    "ITU.IT_NET_USER_M": {"name": "Male individuals using the Internet", "min": 0, "max": 100},
    "ITU.IT_NET_USER_F": {"name": "Female individuals using the Internet", "min": 0, "max": 100},
}
...
def load_raw_data():
    if not RAW_FILE.exists():
        raise FileNotFoundError(
            f"{RAW_FILE} does not exist. Run src/ingestion/itu_datahub.py first."
        )

    df = pd.read_csv(RAW_FILE)
    print(f"Raw ITU data loaded: {len(df)}")
    return df
...
def validate_indicators(df):
    ...
    if unknown:
        raise ValueError(f"Unknown ITU indicators found: {unknown}")

    print("All ITU indicators are recognized")
    return df
...
def validate_values(df):
    ...
    if invalid_count:
        raise ValueError(f"Found {invalid_count} invalid ITU values.")

    print("ITU value validation passed")
    return df
...
def save_processed_data(df):
    ...
    print(f"Cleaned ITU data saved to: {PROCESSED_FILE}")


def clean_pipeline():
    print("\n==========================")
    print("ITU DATA HUB CLEANING")
    print("==========================\n")
